...
folders <- c( "data/fst_NOSS_c5_C20_W200K_S50K" , 
              "data/fst_SS10_c8_C11_W200K_S50K" )
...
for( folder in folders ){
    ...
    for( popNumber in popNumbers ){
        ...
        fileFst         <- paste( folder , "/ALL.tsv" ,                      sep = "" )
        filePng         <- paste( folder , "/ALL."    , popNumber , ".png" , sep = "" )
        fileFstFiltered <- paste( folder , "/ALL."    , popNumber , ".tsv" , sep = "" )
        
        
        data <- readFst( filename = fileFst , popNumber = popNumber )
        
        ul1 <- sort( data$fst )[ nrow(data) * 0.98 ] ; print(ul1)
        
        dataFiltered <- data[ data$fst >= ul1 , 1:6 ] # Save only the first 6 columns, the seventh is the color
        dataFiltered <- na.exclude( dataFiltered )
        
        write.table( x         = dataFiltered    ,
                     file      = fileFstFiltered ,
                     quote     = F               ,
                     row.names = F               ,
                     col.names = F               ,
                     sep       = "\t"            )
        
        plotFst( data = data , fileout = filePng , hlines = ul1 )
