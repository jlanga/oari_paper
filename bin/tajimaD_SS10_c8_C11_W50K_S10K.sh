...
analysis=tajimaD   # Choose between "pi", "D", "theta"
min_count=2
min_coverage=5
max_coverage=20
...
parallel -j $num_cpu                                                                            \
    perl ./src/popoolation_1.2.2/Variance-sliding.pl                                            \
        ...
        --step-size             $step_size                                                      \
        --output                ${vs_folder}/{2}/{2}.{1}.tsv                                    \
        --snp-output            ${vs_folder}/{2}/{2}.{1}.snps                                   \
        "&>"                    ${vs_results}/{2}/{2}.{1}.out                               ";" \
    gzip -9 ${vs_folder}/{2}/{2}.{1}.tsv                                                    ";" \
...
for population in ${populations[@]} ; do
    ...
    # Note that WE ARE LEAVING CHROMOSOME X OUT

    gzip -dc ${vs_folder}/${population}/${population}.{1..26}.tsv.gz |
    parallel --keep-order --pipe                                     \
        bash bin/variance_sliding_to_genomic_score.sh                \
    > ${vs_folder}/${population}.tsv
